...
async def tavily_search(
    ...
    """
    # 🔍 EARLY DEBUG: Function entry
    logger.info("🔍 EARLY DEBUG: tavily_search function STARTED")
    logger.info(f"🔍 EARLY DEBUG: queries={queries}, max_results={max_results}")
    # Step 1: Execute search queries asynchronously
    
    logger.info("🔍 EARLY DEBUG: About to call tavily_search_async")
    ...
    logger.info(f"🔍 EARLY DEBUG: tavily_search_async RETURNED, type={type(search_results)}, len={len(search_results) if search_results else 0}")
    if search_results:
        logger.info(f"🔍 EARLY DEBUG: First result keys: {list(search_results[0].keys()) if search_results[0] else []}")
    ...
    search_log = {
        ...
        "parameters": {
            "max_results": max_results,
            "topic": topic,
            "include_raw_content": True  # Fixed: use literal value instead of undefined variable
        },
